...
- **Синтаксис:** `LOAD addr`
- **Описание:** Загружает значение из памяти по адресу `addr` в аккумулятор (ACC).
- **Операция:** `ACC ← M[addr]`
...
- **Описание:** Сохраняет содержимое аккумулятора в память по адресу `addr`.
- **Операция:** `M[addr] ← ACC`
...
Память команд. Машинное слово -- 5 бит(команда). Реализуется списком словарей, описывающих инструкции (одно слово -- одна ячейка).
...
# Микрокоманды
Размер микрокоманды 24 бита,

|Бит  | Сигнал (- поле)            | Допустимые коды                                     |
|-----|----------------------------|-----------------------------------------------------|
|23   | data address latch         | 1 = защёлкнуть Data Adr                             |
|22   | memory latch               | 1 = ACC → MEM[Data Adr]                             |
|21   | data registry latch        | 1 = DR  ← MEM[Data Adr]                             |
|20   | output latch               | 1 = ACC → OUTPUT-порт                               |
|19   | IP latch                   | 1 = записать новое IP                               |
|18   | address selector           | 0 = из opcode, 1 = из DR                            |
|17   | io selector                | 0 = ALU → ACC, 1 = INPUT → ACC  (через ALU)         |
|16-15| cla (левый ALU)            | 00 = 0, 01 = ACC, 10 = SP                           |
|14-13| cld (правый ALU)           | 00 = 0, 01 = DR, 10 = IP                            |
|12-11| IP selector                | 00 = IP 01 = CU                                     |                         
|10-8 | alu control                | 000 ADD · 001 SUB · 010 MUL · 011 DIV · 100 INC · 101 DEC |
|7-6  | cond                       | режим работы модуля условий                         |
